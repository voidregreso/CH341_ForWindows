/*
 * CH341 Driver request queue routines
...
static NTSTATUS NTAPI CH341QueueInsertIrp(_In_ PIO_CSQ Csq,
        _In_ PIRP Irp,
        _In_ PVOID InsertContext);
_Function_class_(IO_CSQ_REMOVE_IRP)
_IRQL_requires_(DISPATCH_LEVEL)
_Requires_lock_held_(CONTAINING_RECORD(Csq, QUEUE, Csq)->QueueSpinLock)
static VOID NTAPI CH341QueueRemoveIrp(_In_ PIO_CSQ Csq,
                                      _In_ PIRP Irp);
_Function_class_(IO_CSQ_PEEK_NEXT_IRP)
_IRQL_requires_(DISPATCH_LEVEL)
_Requires_lock_held_(CONTAINING_RECORD(Csq, QUEUE, Csq)->QueueSpinLock)
static PIRP NTAPI CH341QueuePeekNextIrp(_In_ PIO_CSQ Csq,
                                        _In_opt_ PIRP Irp,
...
static VOID NTAPI CH341QueueAcquireLock(_In_ PIO_CSQ Csq,
...
static VOID NTAPI CH341QueueReleaseLock(_In_ PIO_CSQ Csq,
                                        _In_ _IRQL_restores_ KIRQL OldIrql);
_Function_class_(IO_CSQ_COMPLETE_CANCELED_IRP)
_IRQL_requires_max_(DISPATCH_LEVEL)
static VOID NTAPI CH341QueueCompleteCanceledIrp(_In_ PIO_CSQ Csq,
        _In_ PIRP Irp);

NTSTATUS CH341InitializeQueue(_In_ PQUEUE Queue);

#ifdef ALLOC_PRAGMA
#pragma alloc_text(PAGE, CH341InitializeQueue)
#endif /* defined ALLOC_PRAGMA */

NTSTATUS
CH341InitializeQueue(
    _In_ PQUEUE Queue) {
    NTSTATUS Status;
    PAGED_CODE();
    Status = IoCsqInitializeEx(&Queue->Csq,
                               CH341QueueInsertIrp,
                               CH341QueueRemoveIrp,
                               CH341QueuePeekNextIrp,
                               CH341QueueAcquireLock,
                               CH341QueueReleaseLock,
                               CH341QueueCompleteCanceledIrp);
